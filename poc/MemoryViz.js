...
const fileInput = body.append('input')
  ...
  // New input and button for loading files from an absolute path
  const pathInput = body.append('input')
    .attr('type', 'text')
    .attr('placeholder', 'Enter absolute path to file');

  const loadFileButton = body.append('button')
    .text('Load File from Path')
    .on('click', () => {
      const filePath = pathInput.node().value;
      if (filePath) {
        logDebug(`Loading file from path: ${filePath}`);
        loadFileFromPath(filePath);
      } else {
        logDebug('No path provided');
      }
    });

  function loadFileFromPath(filePath) {
    logDebug('Fetching file from server...');
    fetch(`/load-file?path=${encodeURIComponent(filePath)}`)
      .then(response => {
      ...
      .then(data => {
        ...
        add_snapshot(filePath, unique_name => {
      ...
      .catch(error => {
        logDebug(`Error loading server file: ${error.message}`);
      });
  }
