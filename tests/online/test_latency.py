...
@pytest.fixture
def mbpp_example():
    return {"text": "Write a Python function to get the unique elements of a list.", "test_list": ["assert get_unique_elements([1, 2, 3, 2, 1]) == [1, 2, 3]"]}
...
def test_get_prompt_samsum(samsum_example):
    ...
    expected = "Summarize this dialog:\nSpeaker 1: Hello\nSpeaker 2: Hi\n---\nSummary:\n"
...
def test_get_prompt_mbpp(mbpp_example):
    ...
    expected_start = "[INST]Your task is to write a Python function to solve a programming problem."
...
def test_get_prompt_alpaca_with_input(alpaca_example_with_input):
    ...
    expected_start = "Below is an instruction that describes a task, paired with an input that provides further context."
    assert prompt.startswith(expected_start)
...
def test_get_prompt_alpaca_without_input(alpaca_example_without_input):
    ...
    expected_start = "Below is an instruction that describes a task. Write a response that appropriately completes the request."
    assert prompt.startswith(expected_start)
